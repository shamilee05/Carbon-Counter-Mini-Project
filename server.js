...
//Creating a RESTful web service which involves dealing with the data of users and involving JWT tokens
...
// Generating an Auth Token for a new user
UserSchema.methods.generateAuthToken = function () {
	var user = this;
	var access = 'authorized';
	var token = jwt.sign({_id : user._id.toHexString(), access}, 'carbon_counter').toString();
...
// Find a user using their assigned Auth Token
UserSchema.statics.findByToken = function(token) {
	var User = this;
	var decoded;

	try{
		decoded = jwt.verify(token , 'carbon_counter');
	}
	catch(e) {
        // Do nothing
	}
	...
	return User.findOne({
		'_id' : decoded._id,
		'tokens.token' : token,
		'tokens.access' : 'carbon_counter'
	});
};

// Creating a new user: POST (Create)
app.post('/users', (req,res) => {
	var body = _.pick(req.body, ['firstname','lastname','username','password']);
	var user = new UserModel(body);

	user.save().then(() => {
		return user.generateAuthToken();    // Generate an AuthToken for the new user
	}).then((token) => {
		res.header('x-auth',token).send(user);  // Add token header to the response
	}).catch( (err) =>{
		res.status(400).send(err);
	});
});

// Reading info about the users: GET (READ)
